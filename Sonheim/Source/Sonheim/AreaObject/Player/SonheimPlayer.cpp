...
void ASonheimPlayer::Tick(float DeltaTime)
{
	...
	if (GetWorld()->GetFirstPlayerController()->WasInputKeyJustPressed(EKeys::T))
	{
		...
		//GetWorld()->GetTimerManager().SetTimer(TimerHandle_SendWav, this, &ASonheimPlayer::SendWavFileDirectly, 6.0f, false);
...
void ASonheimPlayer::SendBinaryDataToServer(const TArray<uint8>& BinaryData)
{
